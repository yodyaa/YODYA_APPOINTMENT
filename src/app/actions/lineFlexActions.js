 ...
 */
// ป้องกันการเรียกใช้จาก client โดยตรง (Next.js server only)
function assertServerEnv() {
    if (typeof window !== 'undefined') {
        throw new Error('sendFlexMessage must be called from the server only!');
    }
}
...
async function sendFlexMessage(userId, flexTemplate, actionName) {
    ...
    // ...existing code...
        ...
        if (!process.env.LINE_CHANNEL_ACCESS_TOKEN) {
            console.error('❌ LINE_CHANNEL_ACCESS_TOKEN not found in environment variables (server only)');
