...
const AppointmentCard = ({ appointment, onCancelClick }) => {
    ...
    // const appointmentDate = appointment.appointmentInfo?.dateTime?.toDate(); // ไม่ต้องแสดงเวลา

    // THEME: สีหลัก #A8999E, ปุ่ม #A8999E, ข้อความเข้ม #333, กล่องเทาอ่อน #F8F8F8, cornerRadius 12px
    return (
        <div className="bg-white rounded-xl shadow border border-gray-200 p-4 space-y-3 text-base">
            <div className="flex justify-between items-center">
                <div className="font-bold text-[#333] text-lg truncate">{appointment.customerInfo?.fullName || appointment.customerInfo?.name}</div>
                <span className={`px-3 py-1 text-xs font-semibold rounded-full ${statusInfo.color}`}>{statusInfo.label}</span>
            </div>
            <div className="bg-[#F8F8F8] rounded-lg p-3 flex flex-col gap-1">
                <div className="flex items-center gap-2">
                    <span className="font-semibold text-[#A8999E]">บริการ:</span>
                    <span className="text-[#333]">{appointment.serviceInfo?.name}</span>
                </div>
                {(appointment.serviceInfo?.price || appointment.appointmentInfo?.price) && (
                    <div className="flex items-center gap-2">
                        <span className="font-semibold text-[#A8999E]">ราคา:</span>
                        <span className="text-[#333]">{(appointment.serviceInfo?.price ? Number(appointment.serviceInfo.price) : Number(appointment.appointmentInfo?.price || 0)).toLocaleString()} {profile.currencySymbol}</span>
                    </div>
                )}
                {addOns.length > 0 && (
                    <div className="flex items-start gap-2">
                        <span className="font-semibold text-[#A8999E]">บริการเสริม:</span>
                        <ul className="list-disc ml-4 text-[#333] text-sm">
                ...
                <div className="flex items-center gap-2">
                    <span className="font-semibold text-[#A8999E]">รวมเวลา:</span>
                    <span className="text-[#333]">{totalDuration ? `${totalDuration} นาที` : '-'}</span>
                </div>
            </div>
            <div className="flex justify-end gap-2 mt-2">
                <button onClick={() => navigateToDetail(router, appointment.id)} className="text-xs bg-[#A8999E] hover:bg-[#8c7a8a] text-white font-semibold py-1.5 px-4 rounded-lg transition">รายละเอียด</button>
                {['awaiting_confirmation', 'confirmed', 'in_progress'].includes(appointment.status) && (
                    <button onClick={() => onCancelClick(appointment)} className="text-xs bg-red-100 hover:bg-red-200 text-red-800 font-semibold py-1.5 px-4 rounded-lg transition">ยกเลิก</button>
                )}
            </div>
        </div>
    );
