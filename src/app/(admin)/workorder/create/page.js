...
export default function CreateWorkorderPage() {
  ...
  const handleCreateWorkorderFromBooking = async (booking, beauticianId, caseNumber, date, time, price) => {
    try {
                ...
                {(() => {
                  ...
                  return (
                    <>
                      ...
                      <div className="text-sm text-gray-700 mb-1"><b>เวลา:</b> {b.date || (b.appointmentInfo?.dateTime?.toDate?.() ? format(b.appointmentInfo.dateTime.toDate(), 'dd MMM yy, HH:mm') : '') || 'ไม่ระบุวันที่'} {b.time || b.dateTime || ''}</div>
