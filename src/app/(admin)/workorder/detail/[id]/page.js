          ...
          <div className="space-y-2">
            ...
            <div className="mb-4">
              ...
              <ol className="relative border-l-2 border-gray-300 ml-3">
                {[
                  { key: 'กำลังเดินทาง', color: 'bg-blue-500', status: 'traveling' },
                  { key: 'ถึงที่ทำงาน', color: 'bg-indigo-500', status: 'arrived' },
                  { key: 'เริ่มทำงาน', color: 'bg-yellow-500', status: 'working' },
                  { key: 'เสร็จสิ้น', color: 'bg-green-500', status: 'finished' }
                ].map((step, idx) => {
                  // Map workorder.status to timeline
                  const statusMap = {
                    'กำลังเดินทาง': 0,
                    'ถึงที่ทำงาน': 1,
                    'เริ่มทำงาน': 2,
                  ...
                  const currentIdx = statusMap[workorder.status] ?? -1;
                  const isActive = idx === currentIdx;
                  const isCompleted = idx < currentIdx;
                  return (
                    <li key={step.key} className="mb-6 ml-4">
                      <div className="flex items-center">
                        <span className={`flex items-center justify-center w-5 h-5 rounded-full border-2 ${isActive ? step.color : isCompleted ? 'bg-gray-300' : 'bg-white'} ${isActive ? 'border-blue-500' : 'border-gray-300'}`}></span>
